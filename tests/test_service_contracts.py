...
def test_coqui_constructor_and_generation_contract(tmp_path, monkeypatch):
    import manim_voiceover.services.coqui as coqui
    ...
    monkeypatch.setattr("manim_voiceover.services.coqui.importlib.import_module", lambda name: SimpleNamespace(TTS=FakeTTS))
    monkeypatch.setattr(
        "manim_voiceover.services.coqui.wav2mp3",
        lambda wav_path, output_path: converted.append((wav_path, output_path)),
    )
    monkeypatch.setattr(
        "manim_voiceover.services.coqui.initialize_speech_service",
        lambda service, kwargs, transcription_model=None: (
            initialized.append((service, kwargs.copy(), transcription_model)) or setattr(service, "cache_dir", tmp_path)
        ),
    )

    service = coqui.CoquiService(
        model_name="model",
        config_path="config.json",
        vocoder_path="vocoder",
        vocoder_config_path="vocoder.json",
        progress_bar=False,
        gpu=True,
        speaker_idx=1,
        language_idx=1,
    ...
    assert service.speaker == "speaker-b"
    assert service.language == "tr"
    assert service.init_kwargs == {"custom_kwarg": "value"}
    assert initialized == [(service, {"custom_kwarg": "value"}, None)]

    service.get_cached_result = lambda input_data, cache_dir: None
    service.get_audio_basename = lambda input_data: "coqui-name"
    result = service.generate_from_text("hello <bookmark mark='m'/> world")

    assert tts_calls == [("hello  world", "speaker-b", "tr", tmp_path / "coqui-name.wav")]
    assert converted == [(tmp_path / "coqui-name.wav", str(tmp_path / "coqui-name.mp3"))]
    assert result == {
        "input_text": "hello <bookmark mark='m'/> world",
        "input_data": {"input_text": "hello <bookmark mark='m'/> world", "service": "coqui"},
        "original_audio": "coqui-name.mp3",
    }


def test_coqui_constructor_defaults(tmp_path, monkeypatch):
    import manim_voiceover.services.coqui as coqui
    ...
    assert service.speaker is None
    assert service.language is None
    assert service.init_kwargs == {}
    assert initialized == [(service, {}, None)]
