...
class test(Scene):
    # def setup(self):
    #     logogif2 = logogif.setup(self)

    def construct(self):
        # self.play(logogif2)
        #        config.tex_template = TexTemplateFromFile(tex_compiler="lualatex",
        #        output_format=".pdf", tex_filename =r"C:\Users\Acer\Desktop\NotoSansArabic-hinted\arabic_template2.tex")

        # a = Tex(r"$\frac{a}{bc} +a\times c$",)
        # title = Title(r"حل المعادلة من الدرجة الثانية بمجهول واحد"
        title = Title(
            r"كيف حالكم can you do it ",
            tex_template=ar_tex,  # substrings_to_isolate="a",
            # title=Title("Hello",
            font_size=75,
            include_underline=True,
            tex_environment="justify",
        )
        tex1 = MathTex(rf"{{ 1 }} + \sqrt{{ {{ 5+1 }} }}")
        tex2 = MathTex(r"1+ \frac{\sqrt{ 6 }}{ 555 }", tex_template=ar_tex).next_to(
            tex1, DOWN
        )
        print(len(tex1))
        # title.arrange(DOWN,buff=.2).to_edge(UP)
        # self.add(title,a)
        self.add(tex1, tex2)
        # self.wait()
        # self.play(ReplacementTransform(tex1[1:].copy(),tex2[1:]),TransformMatchingShapes(tex1[0:],tex2[0:]))
        # self.wait()
...
class Logo(Scene):
    def construct(self):
        ...
        logo = SVGMobject("LogoBlack.svg")
        logo.scale(2)
        bg_circle = Circle(radius=20, fill_color="#e0e6e2", fill_opacity=1)
        bg_circle.set_z_index(-3)

        # self.add(bg_circle)

        circle, stars, phi, main_text = logo
        to_black = VGroup(circle, stars, main_text)
        # stars.set_color(BLACK)
        # main_text.set_color(BLACK)
        logo_color = stars.get_color()
        circle.set_color(WHITE).scale(0.6)
        stars.rotate(PI, about_point=circle.get_center())  # .set_color(WHITE)
        self.add_sound("27 Simple Minimal Logo.wav")
        # staff=VGroup(circle,stars,phi,main_text).arrange(DOWN,buff =.5 )
        ...
        # self.wait(0.3)
        self.play(Write(stars), run_time=0.5)
        circle.save_state()
        stars.save_state()
        phi.save_state()
        main_text.save_state()
        # test= [ stars.animate(path_arc = PI ).rotate(8*PI,about_point=circle.get_center()) for i in range(1,10)]\
        # + [stars.animate(path_arc = PI ).rotate(PI,about_point=circle.get_center() ).set_color(logo_color)]
        # print(len(test))
        # animations = AnimationGroup(*(test
        # ),lag_ratio = 0.2 )
        # animations.build_animations_with_timings()
        # animations= AnimationGroup(*[Rotate])
        # stars.animate(path_arc = PI,rate_func=rate_functions.ease_out_expo  ).rotate(PI,about_point=circle.get_center() ).set_color(logo_color),
        animations = AnimationGroup(
            *[
                Write(phi),
                phi.animate(path_arc=PI).rotate(2 * PI, about_point=phi.get_center()),
                Rotate(
                    stars,
                    5 * PI + PI / 8,
                ...
                FadeIn(main_text, shift=0.2 * UP),
            ],
            lag_ratio=0,
        )

        danimations = AnimationGroup(
            *[
                phi.animate(path_arc=PI).rotate(-2 * PI, about_point=phi.get_center()),
                Unwrite(phi),
                Rotate(
                ...
                FadeOut(main_text, shift=0.2 * DOWN),
            ],
            lag_ratio=0,
        )
        ...
        self.play(
            Circumscribe(circle.copy().scale(0.5), Circle, True, color=RED),
            Indicate(phi, scale_factor=3, color=RED),
            Wiggle(stars),
            Indicate(main_text, color=RED, scale_factor=3),
        )
        self.wait(5.5)

        self.play(
            FadeIn(bg_circle),
            logo.animate.scale(0.5).set_opacity(0.4).to_edge(DL, buff=0.3),
        )
        to_black.set_z_index(-2)
        self.play(to_black.animate.set_color(BLACK), run_time=0.2)
        self.wait(1)


class test1(VoiceoverScene):
    def construct(self):
        ...
        # main_text.set_color(BLACK)
        logo_color = stars.get_color()
        circle.set_color(BLACK).scale(0.6)
        stars.rotate(
            4 * PI + PI / 8, about_point=circle.get_center()
        )  # .set_color(WHITE)
        # stars,5*PI+PI/8, about_point=circle.get_center()
        logo.scale(0.5).set_opacity(0.4).to_edge(DL, buff=0.3)
        to_black.set_z_index(-2)
        to_black.set_color(BLACK)
        self.add(logo)
        self.wait(1)
